        ...
        implements AccessControl
{
    ...
    {
        denySelectColumns(tableName.toString(), columnNames.stream().map(Name::getLegacyName).collect(Collectors.toSet()));
    }
    ...
    {
        denyGrantRoles(roles.stream().map(Name::getCaseNormalizedName).collect(Collectors.toSet()), grantees);
    }
    ...
    {
        denyRevokeRoles(roles.stream().map(Name::getCaseNormalizedName).collect(Collectors.toSet()), grantees);
    }
    ...
    {
        denyShowCurrentRoles(catalogName.getCaseNormalizedName());
    }

    @Override
    public void checkCanShowRoleGrants(TransactionId transactionId, Identity identity, Name catalogName)
    {
        denyShowRoleGrants(catalogName.getCaseNormalizedName());
    }
}
