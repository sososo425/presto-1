...
final class FilterUtil
{
    ...
    {
        QualifiedTablePrefix prefix = new QualifiedTablePrefix(createNonDelimitedName(catalog));
        if (schema.isPresent()) {
            prefix = new QualifiedTablePrefix(createNonDelimitedName(catalog), createNonDelimitedName(schema.get()));
            if (table.isPresent()) {
                prefix = new QualifiedTablePrefix(createNonDelimitedName(catalog), createNonDelimitedName(schema.get()), createNonDelimitedName(table.get()));
            }
        }
        return prefix;
    }
